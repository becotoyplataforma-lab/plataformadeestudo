/**
 * Admin — Financeiro: Reativar assinatura
 *
 * POST /api/admin/financeiro/assinaturas/[id]/reactivate (admin)
 */
...
export async function POST(
  ...
  try {
    ...
    const row = await AdminFinanceService.reactivateSubscription(admin, id);
    ...
    console.error("[admin/financeiro] Erro ao reativar:", error);
    return NextResponse.json(
      { error: "Erro interno", message: "Falha ao reativar assinatura." },
      { status: 500 }
    );
  }
}
